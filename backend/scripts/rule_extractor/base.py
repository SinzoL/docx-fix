...
def main():
    """CLI 入口"""
    if len(sys.argv) < 2:
        print("用法: python -m scripts.rule_extractor <模板docx文件> [--output <yaml路径>] [--name <规则名称>]")
        print()
        print("示例:")
        print("  python -m scripts.rule_extractor template.docx")
        print("  python -m scripts.rule_extractor template.docx --output rules/my_rules.yaml")
        print("  python -m scripts.rule_extractor template.docx --name '我的学校论文格式规则'")
        sys.exit(1)
