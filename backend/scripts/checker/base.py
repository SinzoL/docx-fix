...
用法：
    python -m scripts.checker.base <docx文件路径> [--rules <yaml规则文件>]
"""
...
class DocxChecker:
    ...
    def __init__(self, filepath, rules_path=None):
        ...
        # 初始化属性解析器（延迟导入避免循环引用）
        from scripts.checker.property_resolver import PropertyResolver
    ...
    def run_all_checks(self):
        ...
        from scripts.checker.style_checker import (
        ...
        from scripts.checker.heading_validator import (
        ...
        from scripts.checker.numbering_checker import (
...
def main():
    if len(sys.argv) < 2:
        print("用法: python -m scripts.checker.base <docx文件路径> [--rules <yaml规则文件>]")
        sys.exit(1)
