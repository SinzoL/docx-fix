/**
 * PolishHistoryList — 润色历史记录列表
 *
 * 功能：
 * - 从 IndexedDB 读取润色历史记录
 * - 展示文件名、润色时间、建议数量、是否已应用
 * - 点击可查看历史润色结果
 * - 支持删除单条记录
 */
...
export default function PolishHistoryList({ onViewResult, refreshKey }: PolishHistoryListProps) {
  ...
  const loadHistory = useCallback(async () => {
    setLoading(true);
    try {
      const list = await getPolishHistoryList();
      setRecords(list);
    } catch {
      // IndexedDB 不可用
    } finally {
  ...
  if (loading) {
    return (
      <div className="glass-card rounded-2xl p-8 text-center">
        ...
        <p className="text-sm text-slate-500 font-medium">加载润色历史...</p>
      </div>
    );
  }
    ...
    return (
      <div className="glass-card rounded-2xl p-8 text-center border-dashed border-2 border-slate-300">
        <div className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-3">
          <SvgIcon name="edit" size={24} className="opacity-40" />
        </div>
        <p className="text-base font-bold text-slate-700 font-display mb-1">暂无润色记录</p>
        <p className="text-sm text-slate-500">润色完成后，结果将自动保存在浏览器本地</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-2">
        <h3 className="text-xl font-bold text-slate-800 font-display flex items-center gap-2">
          <span className="text-violet-500"><SvgIcon name="edit" size={20} /></span>
          润色历史 ({records.length})
        </h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {records.map((record) => {
          ...
          return (
            <div
              key={record.id}
              className="glass-card rounded-xl p-5 hover:shadow-lg hover:shadow-violet-500/10 hover:-translate-y-1 transition-all duration-300 cursor-pointer group border-white/60 relative overflow-hidden"
              onClick={() => onViewResult?.(record)}
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-violet-500/5 to-transparent rounded-bl-full pointer-events-none transition-opacity opacity-0 group-hover:opacity-100"></div>
